...
**Business Need**: Track online order flow from creation through pick and ship across stores and DCs.

**Capabilities**:
- Online order volume and average order value
- Fulfillment pipeline status (created → picked → shipped)
- Fulfillment mode mix (SHIP_FROM_STORE, SHIP_FROM_DC, BOPIS)
- Node-level throughput (store vs DC fulfillment)

**KQL Queries**: `fabric/querysets/q_online_orders_15m.kql`, `fabric/querysets/q_fulfillment_pipeline_24h.kql`
**Event Types**: `online_order_created`, `online_order_picked`, `online_order_shipped`

---

## 7. CPG Supplier Collaboration Portal (Future)
