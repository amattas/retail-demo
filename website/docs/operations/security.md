...
:::note
The examples below are illustrative. The shipped `dim_stores` table has no `manager_email` column — to filter by user you would add a manager-mapping table (or column) to the model first. Region lives in `dim_geographies[Region]`, related to stores via `dim_stores[GeographyID]`.
:::

**Example: Store-level access** (requires adding a `manager_email` mapping to `dim_stores`)
```dax
...
role: StoreManagers
  tablePermission:
    table: dim_stores
    filterExpression:
      dim_stores[manager_email] = USERPRINCIPALNAME()
```

**Example: Region-level access** (filters propagate to facts via dim_stores → dim_geographies)
```dax
role: RegionalManagers
  tablePermission:
    table: dim_geographies
    filterExpression:
      dim_geographies[Region] = "West"
```
