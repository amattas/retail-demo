...
The generator itself has no built-in Key Vault integration; it reads `AZURE_EVENTHUB_CONNECTION_STRING` from the environment. Fetch the secret from Key Vault and inject it at startup:

```bash
export AZURE_EVENTHUB_CONNECTION_STRING="$(az keyvault secret show \
  --vault-name retail-datagen-kv \
  --name eventhub-connection-string \
  --query value -o tsv)"
./launch.sh
```
