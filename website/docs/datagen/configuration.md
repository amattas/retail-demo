...
:::note
`AZURE_EVENTHUB_CONNECTION_STRING` takes priority over `realtime.azure_connection_string` in `config.json` when both are set.
:::

The configuration can also be read and updated at runtime via the API: `GET /api/config`, `PUT /api/config`, `POST /api/config/reset`, `POST /api/config/validate`.
...
| `refrigerated_trucks` | int | 8 | Refrigerated trucks (DC-to-store) |
| `non_refrigerated_trucks` | int | 12 | Non-refrigerated trucks (DC-to-store) |
| `supplier_refrigerated_trucks` | int | 12 | Refrigerated trucks (supplier-to-DC) |
| `supplier_non_refrigerated_trucks` | int | 18 | Non-refrigerated trucks (supplier-to-DC) |
| `truck_capacity` | int | 15,000 | Max items per truck |
| `truck_dc_assignment_rate` | float | 0.85 | % trucks assigned to DCs |
| `trucks_per_dc` | int\|null | null | Override automatic truck-to-DC assignment |
| `dc_initial_inventory_min` / `_max` | int | 500 / 5,000 | Initial inventory per product at DCs |
| `store_initial_inventory_min` / `_max` | int | 20 / 200 | Initial inventory per product at stores |
| `dc_reorder_point_min` / `_max` | int | 50 / 500 | DC reorder point range |
| `store_reorder_point_min` / `_max` | int | 5 / 50 | Store reorder point range |
...
|-------|------|-------------|
| `dictionaries` | string | Path to dictionary CSV files (`dict` is accepted as an alias) |
...
```json
{
  "marketing_cost": {
    ...
    "facebook_cost_max": 3.00,
    "google_cost_min": 1.00,
    "google_cost_max": 5.00,
    "instagram_cost_min": 0.20,
    "instagram_cost_max": 3.00,
    "youtube_cost_min": 1.50,
    "youtube_cost_max": 5.25,
...
Per-channel cost ranges exist for nine channels (email, display, social, search, video, facebook, google, instagram, youtube), each with `<channel>_cost_min` / `<channel>_cost_max`. The actual cost per impression is a random value in the channel range multiplied by the device multiplier.
