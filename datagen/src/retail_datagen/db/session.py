...
transaction handling with master, facts, and unified retail databases.
"""
...
_retail_session_maker: async_sessionmaker[AsyncSession] | None = None
...
def retail_session_maker() -> async_sessionmaker[AsyncSession]:
    """
    Get or create session maker for unified retail database.

    The session maker is a factory for creating AsyncSession instances.
    It's configured with appropriate defaults for the retail database.

    Returns:
        async_sessionmaker configured for retail database
    ...
    Example:
        ...
        ...     result = await session.execute(select(Store))
    """
    global _retail_session_maker

    if _retail_session_maker is None:
        engine = get_retail_engine()
        _retail_session_maker = async_sessionmaker(
            bind=engine,
            class_=AsyncSession,
            autoflush=True,
            autocommit=False,
        ...
        logger.debug("Created retail database session maker")

    return _retail_session_maker
...
@asynccontextmanager
async def get_retail_session() -> AsyncIterator[AsyncSession]:
    """
    Context manager for retail database sessions with automatic transaction handling.

    This context manager:
    - Creates a new session from the retail session maker
    - Automatically commits on successful completion
    - Automatically rolls back on exception
    - Ensures proper cleanup

    Yields:
        AsyncSession for retail database operations
    ...
    async with SessionMaker() as session:
        try:
            yield session
            await session.commit()
            logger.debug("Retail session committed successfully")
        except Exception as e:
            await session.rollback()
            logger.error(f"Retail session rolled back due to error: {e}")
            raise
        finally:
            await session.close()
...
async def get_session(database: str = "master") -> AsyncIterator[AsyncSession]:
    ...
    Args:
        database: Either "master", "facts", or "retail" to specify target database

    Yields:
        AsyncSession for specified database

    Raises:
        ValueError: If database parameter is not "master", "facts", or "retail"
...
class SessionContext:
    ...
    def __init__(self, database: str = "master"):
        ...
        Args:
            database: Either "master", "facts", or "retail"
        """
        if database == "master":
            ...
            self._session_maker = retail_session_maker()
        else:
