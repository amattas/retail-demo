...
def ensure_fact_store_ops_table(conn: duckdb.DuckDBPyConnection) -> None:
    ...
    Schema includes only the core payload fields generated by store_ops_mixin.
    Envelope fields (event_type, ingest_timestamp, schema_version, etc.) are added
    by the streaming layer when events are wrapped in EventEnvelope objects and
    appear only in the KQL tables after streaming ingestion.
    """
    conn.execute(
        """
    ...
    try:
        ...
        # Ensure core payload fields exist for backward compatibility
