...
def find_class_or_function(lines: List[str], name: str, start_idx: int = 0) -> Tuple[int, int]:
    ...
    for i in range(start_idx, len(lines)):
        ...
        if re.match(rf'^class {re.escape(name)}[\(:< ]', lines[i]) or \
...
convenience_start, convenience_end = find_class_or_function(lines, 'generate_historical_facts', fact_gen_end)
if convenience_start:
    print(f"generate_historical_facts function: lines {convenience_start + 1} to {convenience_end}")
