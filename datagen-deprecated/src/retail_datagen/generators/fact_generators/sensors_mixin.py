...
class SensorsMixin(FactGeneratorBase):
    ...
    def _generate_foot_traffic(
        ...
        noise = np.array(
            [0 if v <= 0 else self._np_rng.integers(-v, v + 1) for v in variance],
            dtype=np.int32,
        )
        ...
        try:
            ...
            logger.warning(
                f"Failed to create BLE pings via pandas, using fallback: {e}"
            )
            return [
                {
    ...
    def _generate_ble_pings(
        ...
        """
        zones_all = np.array(
            ["ENTRANCE", "ELECTRONICS", "GROCERY", "CLOTHING", "CHECKOUT"], dtype=object
        )
        beacons_all = np.array(
            [f"BEACON_{store.ID:03d}_{z}" for z in zones_all], dtype=object
        )
        ...
        pick_idx = np.array(
            self._np_rng.choice(len(zones_all), size=k, replace=False, p=(w / w.sum()))
        )

        # Base per-zone pings and multiplier by format and traffic
        per_zone_counts = self._np_rng.integers(
            2, 6, size=k
        )  # 2-5 pings per chosen zone
        fmt_mult_map = {
            "hypermarket": 1.6,
            "superstore": 1.3,
            "standard": 1.0,
        ...
        per_zone_counts = np.maximum(
            1, (per_zone_counts.astype(float) * ble_mult)
        ).astype(np.int32)
        ...
        try:
            ...
            logger.warning(
                f"Failed to create foot traffic via pandas, using fallback: {e}"
            )
            out: list[dict] = []
                ...
                out.append(
                    {
                        ...
                        + timedelta(minutes=int(offsets[i])),
                        "StoreID": store.ID,
