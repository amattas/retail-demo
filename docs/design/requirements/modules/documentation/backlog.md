# Documentation backlog

## Open

### IMP-015 - Derive docs, asset inventory, and onboarding from one manifest {#imp-015}

- **Priority / effort:** P2 / L
- **Outcome:** One machine-readable inventory drives or validates CLI examples,
  schemas, deploy groups, profiles, prerequisites, support status, and item
  descriptions.
- **Acceptance:** Public onboarding, deployment output, workspace inventory, and
  model documentation agree and pass docs-contract tests.

## Settled — do not reopen

- Canonical site source is `docs/`; `website/docs/` is retired.
- Zensical replaces Docusaurus.
- Generated site output is pushed to `gh-pages`.
- Versioned documentation shows `main` as **Latest** plus the highest tagged
  patch revision for each SemVer minor line.
- Backlogs contain only open work and settled decisions, never completed-work
  archives.
- Dated implementation plans are removed after durable content is promoted.
